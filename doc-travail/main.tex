...
\If{$h < \epsilon$ \OR $| \Omega_{\alpha} | < n_{min}$ \OR $\forall \omega_i,
	...
	\State \textbf{return} $\{c\}$ tel que $|\{\omega_i \in \Omega_{\alpha}: \lambda(\omega_i) = c\}| \geq |\{\omega_i \in \Omega_{\alpha}: \lambda(\omega_i) \neq c\}|$
\EndIf
...
\State $\Gamma_{\leq}\gets$ BUILD\_DT($\Omega_{\leq}, H^*, H, \epsilon, \Delta, n_{min}, \lambda, \delta +1$)
\State $\Gamma_{\geq}\gets$ BUILD\_DT($\Omega_{\geq}, H^*, H, \epsilon, \Delta, n_{min}, \lambda, \delta +1$)
...
\State $E \gets \varnothing $ \Comment{valeurs de $H^*$ obtenues pour chaque seuil de coupure}
...
\State \textbf{return} $\min E, arg\,min_{\mathcal{s} \in S} E $ \Comment{retourner la valeur d'entropie minimale et le seuil de coupure permettant de l'obtenir}
\EndProcedure
\end{algorithmic}
\end{algorithm}
