...
\For{$v \in U$}
    ...
    \State $\lambda_{v} \gets \{\lambda_i : \exists \omega_h \in G, \lambda(\omega_h) = \lambda_i\}$
    \State $v' \gets$ valeur suivante dans U
    \State $\Omega_{v'} \gets \{\omega_i \in \Omega : a_j(\omega_i) = v'\}$
    \State $\lambda_{v'} \gets \{\lambda_i : \exists \omega_h \in G', \lambda(\omega_h) = \lambda_i\}$
    \State $V \gets V \cup \{G\}$    
    ...
    \If{$\lambda_{G'} \neq \lambda_G$}
        ...
        \State $seuils \gets$ seuils $\cup \{x_{j_s}\}$
        \State $h \gets$ h $\cup \{H^*(\lambda | a^{b}_j)\}$
    \EndIf
\EndFor

\State \textbf{return} $\min h, arg\,min_{s \in seuils} h$
\EndProcedure
\end{algorithmic}
\end{algorithm}
