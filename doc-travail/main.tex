...
\begin{algorithmic}
\Procedure{build\_dt}{$\Omega_{\alpha}$, $H^*$, H, $\epsilon$, $\Delta$, $n_min$, $\lambda$, $\delta$}
...
\State \Comment{$n_min$ : taille minimale pour $\Omega_{\alpha}$}
...
\If{$h < \epsilon$ \OR $| \Omega_{\alpha} | < n_min$ \OR $\forall \omega_i,
    \omega_h \in \Omega_{\alpha}, \lambda(\omega_i) = \lambda(\omega_h)$ \OR $\delta > \Delta$}
	\State \textbf{return} $\{c\}$ tel que $|\{\omega_i \in \Omega_{\alpha}: \lambda(\omega_i) = y\}| \geq |\{\omega_i \in \Omega_{\alpha}: \lambda(\omega_i) \neq y\}|$
\EndIf
...
\State $\Gamma_{\leq}\gets$ BUILD\_DT($\Omega_{\leq}, H^*, H, \epsilon, \Delta, n_min, \lambda, \delta +1$)
\State $\Gamma_{\geq}\gets$ BUILD\_DT($\Omega_{\geq}, H^*, H, \epsilon, \Delta, n_min, \lambda, \delta +1$)
...
\State $\mathcal{S}\gets \varnothing $ \Comment{seuils de coupure considérés}
\State $\mathcal{E}\gets \varnothing $ \Comment{valeurs de $H^*$ obtenues pour chaque seuil de coupure}
...
\For{$v \in U$}
    ...
    \If{$C_{v'} \neq C_{v}$}
        ...
        \State $\mathcal{S} \gets \mathcal{S} \cup \{x_{j_s}\}$
        \State $\mathcal{E} \gets \mathcal{E} \cup \{H^*(\lambda | a^{b}_j)\}$
    \EndIf
\EndFor

\State \textbf{return} $\min \mathcal{E}, arg\,min_{\mathcal{s} \in \mathcal{S}} \mathcal{E}$ \Comment{retourner la valeur d'entropie minimale et le seuil de coupure permettant de l'obtenir}
\EndProcedure
\end{algorithmic}
\end{algorithm}
