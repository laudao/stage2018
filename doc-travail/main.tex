...
d'améliorer la performance du classifieur. Dans \cite{pei-partially},
...
\begin{itemize}
    \item Le RIR ascendant (\textit{upward rank inconstency rate}) selon~$a_j$ :
    ...
    \item Le RIR descendant (\textit{downward rank inconstency rate}) selon~$a_j$ : 
...
Pei et Hu comparent PMDT à DIR-DOMLEM, VC-DomLEM, REMT, RGMT, OLM et OSDL sur 12
bases réelles pré-traitées (normalisation des attributs, suppression des données
incomplètes, transformation des attributs croissants avec la classe en attributs
décroissants) dans lesquelles le sens de variation des critères n'est pas connu. 
